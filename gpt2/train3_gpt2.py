...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # not really a bias, more of a mask, but follows the OpenAI/HF naming though
    ...
    def forward(self, x):
        ...
        # attention (materializes the largest (T,T) matrix for all the queries and keys)
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        try:
            ...
            print(f"Error: {e}")
            print("\nPlease run 'download_model.py' first to download the GPT-2 weights.")
            raise
        ...
        sd_keys_hf = [k for k in sd_keys_hf if not k.endswith('.attn.masked_bias')] # ignore this mask / buffer in the HF model as well
